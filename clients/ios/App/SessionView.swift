...
struct SessionView: View {
    ...
    /// Geometry measured during layout. Nothing in `body` reads these, so they
    /// deliberately live outside view state: they are written from preference
    /// and GeometryReader callbacks, which run mid-layout, and a `@State` write
    /// there forces a second update pass in the same frame. That pass re-emits
    /// the bottom preference and re-fires `onChange(of: transcriptRevision)`
    /// if a streamed event landed in between — the "tried to update multiple
    /// times per frame" runtime warnings.
    ...
    @State private var measured = TranscriptGeometry()
    ...
    /// frame".
    private final class ScrollToken {
        var value = 0
        /// A post-layout jump is queued but has not run yet. Further requests
        /// before it runs are redundant: it scrolls to whatever the bottom is by
        /// then. Skipping them avoids one forced layout flush per streamed event.
        var pending = false
    }
    ...
    private func transcript(proxy: ScrollViewProxy) -> some View {
        ...
        .onPreferenceChange(TranscriptTopPreferenceKey.self) { topY in
            ...
            if moved, dragging, !measured.latestVisible {
            ...
            DragGesture(minimumDistance: 4)
                .onChanged { value in
                    ...
                    if !dragActivity.watchdogScheduled {
                        ...
                        Task { @MainActor in
                            ...
                            if measured.latestVisible || dragActivity.sawLiveEdge,
                               !isFollowingLatest, !isBrowsingEarlier {
                                isFollowingLatest = true
                                if !measured.latestVisible {
                                    requestScrollToLatest(proxy: proxy)
                                }
                            }
                        }
                    ...
                    if !measured.latestVisible || value.translation.height > 0 {
                        dragActivity.sawLiveEdge = false
                        stopFollowingLatest()
                    }
                }
                .onEnded { value in
                    ...
                    if value.predictedEndTranslation.height <= 0,
                       measured.latestVisible || dragActivity.sawLiveEdge,
                       !isFollowingLatest, !isBrowsingEarlier {
                        isFollowingLatest = true
                        requestScrollToLatest(proxy: proxy)
                    }
        ...
        .background {
            GeometryReader { reader in
                Color.clear
                    .onAppear { measured.viewportHeight = reader.size.height }
                    .onChange(of: reader.size.height) { _, height in
    ...
    /// Drop any queued jump to the live edge. Clearing `pending` here, rather
    /// than when the stale task wakes, lets the next request queue a fresh jump
    /// immediately instead of being swallowed as a duplicate.
    private func invalidateScrollRequests() {
        scrollToken.value &+= 1
        scrollToken.pending = false
    }
    ...
    private func requestScrollToLatest(proxy: ScrollViewProxy) {
        guard isFollowingLatest, !scrollToken.pending else { return }
        scrollToken.pending = true
        scrollToken.value &+= 1
        let request = scrollToken.value
        Task { @MainActor in
            // Let the transcript, keyboard safe area, and composer all
            // finish the current layout pass. Stopping follow mode bumps the
            // token underneath us and supersedes this request.
            await Task.yield()
            // A superseded request was already cleared by the invalidator.
            guard request == scrollToken.value else { return }
            scrollToken.pending = false
            guard isFollowingLatest else { return }
            proxy.scrollTo(Self.bottomAnchor, anchor: .bottom)
