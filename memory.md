...
- iOS: question rows resolve via openQuestionRowID; composer clear needs the autocorrect-pulse trick; unread badges are client-side watermarks (SessionReadStore); nav pushes go through HomeRouter.open with dedupe; manual keyboard avoidance (App/KeyboardObserver.swift) for bottom-chrome screens (FB13296535).
...
- iOS project rename/removal live only in the drawer rows' long-press context menu — intentional, no visible affordance wanted.
