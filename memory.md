...
- 2026-08-07: Codex stateless replay (task 0197): the turn's Responses output items (reasoning id/encrypted_content/summary + message/function_call ids) ride as ONE marked gollama.ThinkingBlock (Redacted = "codex-response-items-v1:{model,items}") so they persist on model_turn.thinking_blocks and rebuild via ReplayHistory; engine.messagesForBackend strips those blocks for non-openai backends (marker duplicated in loop.go to avoid an engine↔codex test import cycle), and buildInput drops items from a different model or with no following item.
