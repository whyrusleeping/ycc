...
// from baseRef with `git worktree add <dir> -b <branch> <base-commit>`. dir should
// live outside the primary tree.
func (r *Repo) AddWorktree(dir, branch, baseRef string) error {
	_, err := r.run("worktree", "add", dir, "-b", branch, baseRef)
	return err
}

// RemoveWorktree removes the linked worktree at dir. It uses
...
// crashed worktrees with `git worktree prune`.
...
// in the Workstreams panel. Returns 0 on any error.
