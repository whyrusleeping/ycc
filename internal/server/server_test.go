...
// mutations persist to the task file and regenerate backlog.md; invalid inputs are
// rejected; an unknown id is NotFound; and a no-field "refresh" regenerates the
// index without altering the task.
func TestUpdateTask(t *testing.T) {
	...
	indexPath := filepath.Join(ws, "backlog.md")

	srv := New(session.NewManager(reg, ws))
	ctx := context.Background()

	// Status + priority change persists and regenerates the index.
	status := "in_review"
	prio := int32(1)
	...
	if !strings.Contains(string(idx), "in_review") {
		t.Fatalf("backlog.md not regenerated with new status:\n%s", idx)
	}
	...
	// No-field refresh succeeds and regenerates the index (used after $EDITOR).
	if err := os.Remove(indexPath); err != nil {
		t.Fatalf("remove index: %v", err)
	}
	if _, err := srv.UpdateTask(ctx, connect.NewRequest(&v1.UpdateTaskRequest{Id: a.ID})); err != nil {
		t.Fatalf("refresh UpdateTask: %v", err)
	}
	if _, err := os.Stat(indexPath); err != nil {
		t.Fatalf("index not regenerated by refresh: %v", err)
	}
}
